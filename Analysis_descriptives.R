...
table(base_data$selfScoreCat,useNA="always")/20 
base_data$selfScoreCat <- as.factor(base_data$selfScoreCat)
prop.table(table(base_data$selfScoreCat,useNA="always")/20 )
...
## education
table(base_data$education, useNA="always")/20
prop.table(table(base_data$education))

## occupation
table(base_data$occupation, useNA="always")/20
prop.table(table(base_data$occupation)/20)

## table 1 according to subjective smartphone behavior profiles
table(base_data$selfScoreCat, useNA="always")/20
prop.table(table(base_data$selfScoreCat)/20)
...
table(popu$imp_nr, useNA = "always")
popu <- subset(popu,imp_nr!=0)
...
popuAlt <- subset(popuAlt,imp_nr!=0)
...
## SD for pooled mean age
sqrt( mean(aggregate(subset(popu,imp_nr!=0)$agePNR,by=list(subset(popu,imp_nr!=0)$imp_nr),FUN=var)$x)+sum((aggregate(subset(popu,imp_nr!=0)$agePNR,by=list(subset(popu,imp_nr!=0)$imp_nr),FUN=mean)$x-mean(subset(popu,imp_nr!=0)$agePNR))^2)/19
)
...
table(po$imp_nr, useNA = "always")
po <- subset(po,imp_nr==0)
