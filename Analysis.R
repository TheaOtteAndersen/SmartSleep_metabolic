...
sum(!(base_weights$Weights%in% base_data$sample_weights))
...
#All of the id's from base_weights are in use.
...
CSS_track$sample_weights<-as.numeric(left_join(CSS_track,rename(CSS_weights,userid=RespondKey),by="userid")$Weights)
...
hist(simulate(new_model),breaks=10)
